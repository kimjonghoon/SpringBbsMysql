...
출금액이 잔고보다 커도 출금이 되는, 잔고가 음수가 가능한 계좌에 대한 요구 사항이 생겼다.<br />
상속을 적용하면 기존의 계좌를 부모 클래스로, 잔고가 음수여도 되는 계좌를 자식 클래스로 둘 수 있다.<br />
잔고가 음수여도 되는 계좌를 마이너스 계좌라고 부른다면,
마이너스 계좌는 부모 클래스인 계좌 클래스의 출금 메서드 즉, withdraw()만 오버라이딩하면 될 것이다.<br />
...
addAccount(String accountNo, String name, long balance, <strong>String kind</strong>)과 
같이 kind란 파라미터를 추가하고 이 파라미터에 일반 또는 마이너스 란 문자를 받도록 구현하려 한다.<br />
...
금액에 3 자리마다 ,(콤마)가 있도록
입출금 명세 클래스의 toString() 메서드를 수정한다.<br />
...
public class Test {
    static final String ACCOUNTS_HEADING = "계좌번호|소유자명|잔고<strong>|계좌종류</strong>";
...
<h3>과제</h3>
사용자 정의 익셉션을 만들어서 적용하시오.

<ul>
	<li>중복된 계좌 - DuplicateAccountException</li>
	<li>잔고부족 - InsufficientBalanceException</li>
</ul>
