...
주의할 점은 자바에선 배열도 객체라는 사실이다.<br />
따라서 new키워드로 배열 객체를 생성해야 배열을 사용할 수 있게 된다.<br />
...
다음은 배열 변수 arr 를 선언함과 동시에 1,2,3,4,5를 배열에 담는 코드이다.<br />
결과는 위 예제와 같다.<br />
...
<h3>1차원 참조 자료형 배열</h3>
기본 자료형에 대한 예제를 보았으니 참조 자료형에 대한 배열의 예를 보자.<br /> 
...
arr2[0] = "자바";//첫번째 배열 요소에 "자바" String 객체의 참조값을 할당
...
다음은 배열 변수 arr2 를 선언함과 동시에<br />
"자바","스쿨","java","school" 참조값을 배열에 담는 코드이다.<br />
결과는 위 예제와 같다.<br />
...
<h3>2차원 기본 자료형 배열</h3>
2차원 이상의 배열은 배열 요소에 배열 객체의 참조값이 할당된다.<br />
2차원 배열은 수학의 행렬과 비슷하다.<br />
첫번째 꺽쇠가 행을 두번째 꺽쇠는 열을 의미한다.<br />
...
다음은 배열 변수 arr3 를 선언함과 4행 3열의 배열에 차례로 1부터 12까지 배열에 담는 코드이다.<br />
결과는 위 예제와 같다.<br />
...
자바의 배열은 생성하고 난 다음에 배열의 크기를 늘이거나 줄일 수 없다는 단점이 있다.<br />
배열의 인스턴스 변수 length는 배열의 크기(배열 요소의 수)를 저장하고 있는데
배열 객체가 생성될 때 정해지고 이후 변하지 않는다.<br />
자주 하는 혼동을 막기 위해 강조하면,<br />
length에 저장되는 수는 단순히 배열 요소의 갯수이지, 결코 의미있는 값을 가지고 있는 배열 요소가 
몇개인지를 나타내는 수가 아니다.<br />
의미있는 값을 가진 배열 요소가 몇개나 있는지를 나타내는 변수는 배열에 없다.<br />

<dl class="note">
<dt>a.length, a[0].length, a[1].length 값은?</dt>
<dd>
int[][] a; = null;<br />
a = new int[4][];<br />
a[0] = new int[3];<br />
a[1] = new int[7];<br />
System.out.println(a.length);//4<br />
System.out.println(a[0].length);//3<br />
System.out.println(a[1].length);//7<br />
</dd>
</dl>

<h3>main 메소드에 String[] 배열를 인자로 전달하는 방법</h3>
