...
호이스팅이란 자바스크립트 인터프리터가 <strong>"선언된 변수나 함수를 현재 실행 영역의 맨 위로 끌어올리는 것"</strong>을 말한다.<br />
...
위 코드가 호이스팅되면 결과적으로 아래 코드와 같을 것이다.<br />
...
자바 프로그래머에게 이 코드가 문제없이 실행되는 것은 불편하다.<br />
자바에서는 메서드 안에 정의된 지역변수는 메서드가 종료되면 사라진다.<br />
마찬가지로 fn() 함수가 종료될 때 이 함수 내부에 정의된 지역 변수(balance)가 없어지는 것이 
상식적이다.<br />
그런데 fn() 함수가 리턴 값을 반환했는데도 balance 변수는 여전히 유효하다.<br />
<br />
이 문제에 대한 해답은 retFn가 클로저를 갖는다는 것이다.<br />
